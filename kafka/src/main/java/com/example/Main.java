...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        // Record how long each send() takes
        // Report the median in ms
        // ─────────────────────────────────────────
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            ProducerRecord<String, String> record =
                    new ProducerRecord<>("lab4", "key-" + i, value);
        ...
        // Format: "<timestamp_ms>|<padding>"
        // ─────────────────────────────────────────
        System.out.println("\n========== C) PRODUCING FOR LATENCY TEST ==========");

        for (int i = 0; i < 10_000; i++) {
            long sendTime = System.currentTimeMillis();
            String value  = sendTime + "|" + "a".repeat(1004);

            ProducerRecord<String, String> record =
                    new ProducerRecord<>("lab4", "lat-" + i, value);

            producer.send(record); // async → measures real pipeline delay
        }
    ...
    private static long runThroughputTest(KafkaProducer<String, String> producer,
        ...
        while (System.nanoTime() < windowEnd) {
            ProducerRecord<String, String> record =
                    new ProducerRecord<>("lab4", "tp-" + i++, value);
